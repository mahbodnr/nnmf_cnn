...
class CNNmfCnnNetwork(nn.Module):
    def __init__(
        self,
        input_shape,
        nnmf_iterations=5,
        ...
        super(CNNmfCnnNetwork, self).__init__()
        self.conv = CNNmfCnn(
...
@register_model
def cnnmf_cnn_top(**kwargs):
    return CNNmfCnnNetwork(
...
@register_model
def cnnmf_cnn_top_h(**kwargs):
    return CNNmfCnnNetwork(
        input_shape=kwargs.get("input_shape", (3, 32, 32)),
        cnn_features=kwargs.get("cnn_features", [16, 32, 64, 10]),
        cnn_kernel_sizes=[5, 5, 4, 1],
        pooling=[True, True, False, False],
        activation=[nn.ReLU()] * 3 + [nn.Softmax(dim=1)],
        ann_layers=[],
